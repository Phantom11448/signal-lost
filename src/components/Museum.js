    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.textPrimary, fontFamily: FONTS.body, padding: "20px 16px 48px", maxWidth: 580, margin: "0 auto", boxSizing: "border-box", position: "relative", zIndex: 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
        ...
        <div>
          ...
          <div style={{ fontWeight: 700, fontSize: 17, color: C.textPrimary, fontFamily: FONTS.heading, letterSpacing: 1 }}>Mistakes Museum ðŸ›ï¸</div>
        </div>
      </div>
      <p style={{ color: C.textMuted, fontSize: 13, marginBottom: 20, lineHeight: 1.6 }}>
        Even the best human coders make these mistakes. Study them so you don't have to learn the hard way. ðŸ‘¾
      </p>
      ...
      <div style={{ background: C.card, border: `1.5px solid ${C.accent}44`, borderRadius: 14, padding: 20 }}>
        ...
        <h3 style={{ color: C.gold, margin: "0 0 16px", fontSize: 18 }}>ðŸ’€ {m.title}</h3>

        <div style={{ marginBottom: 12 }}>
          <p style={{ color: C.red, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, margin: "0 0 6px" }}>âŒ Wrong</p>
        ...
        <div style={{ marginBottom: 16 }}>
          <p style={{ color: C.alien, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, margin: "0 0 6px" }}>âœ“ Correct</p>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
          <p style={{ color: C.textMuted, margin: 0, fontSize: 13, lineHeight: 1.6 }}>ðŸ”¬ {m.explain}</p>
        </div>

        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16 }}>
          <button onClick={() => setActive((a) => Math.max(0, a - 1))} disabled={active === 0}
            style={{ background: active === 0 ? C.border : C.surface, color: active === 0 ? C.textMuted : C.textPrimary, border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 16px", cursor: active === 0 ? "default" : "pointer", fontSize: 13 }}>â† Prev</button>
          <button onClick={() => setActive((a) => Math.min(MISTAKES.length - 1, a + 1))} disabled={active === MISTAKES.length - 1}
            style={{ background: active === MISTAKES.length - 1 ? C.border : C.accent, color: active === MISTAKES.length - 1 ? C.textMuted : C.bg, border: "none", borderRadius: 8, padding: "7px 16px", cursor: active === MISTAKES.length - 1 ? "default" : "pointer", fontSize: 13, fontWeight: 700 }}>Next â†’</button>
        </div>
      </div>
    </div>
  );
